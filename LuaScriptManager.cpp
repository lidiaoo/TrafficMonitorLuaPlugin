...
string runCmdLine(string cmd)
{
	...
	// 使用 cmd /c 执行命令，支持 && 和环境变量
	CString cmdLine = L"cmd /c " + CString(cmd.c_str());
	TCHAR* cmdBuffer = new TCHAR[cmdLine.GetLength() + 1];
	wcscpy_s(cmdBuffer, cmdLine.GetLength() + 1, cmdLine);
	...
	if (CreateProcess(NULL, cmdBuffer, NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi))
	{
		...
		while (true)
		{
			if (!ReadFile(g_hChildStd_OUT_Rd, buffer, N - 1, &bytesRead, NULL))
			{
		...
		lg << L"failed run cmdline: " << cmdLine << L":" << GetLastError() << endl;
	}
...
wchar_t* multiByteToWideChar(const string& pKey)
{
	...
	int len = strlen(pCStrKey) + 1;
	
	// 优先尝试 UTF-8 解码
	int pSize = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, pCStrKey, len, NULL, 0);
	if (pSize > 0) {
		wchar_t* pWCStrKey = new wchar_t[pSize];
		MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, pCStrKey, len, pWCStrKey, pSize);
		return pWCStrKey;
	}
	
	// UTF-8 失败，回退到系统默认编码（GBK）
	pSize = MultiByteToWideChar(CP_OEMCP, 0, pCStrKey, len, NULL, 0);
	wchar_t* pWCStrKey = new wchar_t[pSize];
	MultiByteToWideChar(CP_OEMCP, 0, pCStrKey, len, pWCStrKey, pSize);
	return pWCStrKey;
}
...
void LuaItem::DrawItem(void* hDC, int x, int y, int w, int h, bool dark_mode)
{
	...
	if (onlyText) {
		...
		for (auto seg : mSegs) {
			// 将 UTF-8 转换为宽字符
			char* pCStr = (char*)seg.text.c_str();
			int len = strlen(pCStr) + 1;
			int pSize = MultiByteToWideChar(CP_UTF8, 0, pCStr, len, NULL, 0);
			wchar_t* pWCStr = new wchar_t[pSize];
			MultiByteToWideChar(CP_UTF8, 0, pCStr, len, pWCStr, pSize);
			text += pWCStr;
			delete[] pWCStr;
		}
		auto multiLine = (h > g_data.DPI(30));
		UINT flag = multiLine ? (DT_NOPREFIX | DT_WORDBREAK) : (DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
		pDC->DrawTextW(text.c_str(), rect, flag);
	}
	else {
		for (auto seg : mSegs) {
			if (seg.hasImage()) {
				...
				CRect textRect = rect;
				textRect.left = x;
				pDC->DrawTextW(pWCStr, textRect, DT_LEFT | DT_SINGLELINE | DT_NOPREFIX);
